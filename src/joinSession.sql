...
select l.* , b.book_name from Library l join Book b on l.book_id = b.id;
-- 5. Получить библиотеки и полные имена читателей
select l.* ,r.full_name from Library l join Reader r on l.reader_id = r.id;
-- 7. Вывести библиотеки, в которых книга сейчас "забронирована" (is_booked = true)
select * from Library l
    join Book b on l.book_id = b.id
         where b.is_booked = true;
...
select r.full_name , l.name ,l.id from Reader r full join Library l on r.id = l.reader_id;
